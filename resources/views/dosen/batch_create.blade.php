...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-black text-[#004d4d] tracking-tighter italic uppercase">
            Import <span class="text-[#2dce89]">Dosen</span>
        </h1>
...
<div class="grid grid-cols-3 gap-6">
    ...
    <div class="col-span-2 bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8">
        ...
        <form method="POST" action="{{ route('dosen.batch.store') }}" enctype="multipart/form-data" id="uploadForm">
            @csrf
    ...
    <div class="flex flex-col gap-5">
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                @foreach([
                    ['nidn',       'Nomor Induk Dosen (unik)',                    'tag'],
                    ['nama_dosen', 'Nama lengkap dosen',                          'person'],
                    ['email',      'Email aktif (unik)',                          'mail'],
                    ['akses_role', 'dosen_pengampu / manager_proyek / keduanya',  'manage_accounts'],
                ] as [$col, $desc, $icon])
                ...
                <div class="mt-4 pt-4 border-t border-gray-100 space-y-1">
                    ...
                    <p class="text-[10px] text-gray-400 font-medium flex items-center gap-1.5">
                        <span class="material-symbols-outlined text-[10px]">info</span>
                        Status akun = active (langsung aktif)
                    </p>
                    <p class="text-[10px] text-gray-400 font-medium flex items-center gap-1.5">
                        <span class="material-symbols-outlined text-[10px]">info</span>
                        NIDN/email duplikat otomatis dilewati
                    </p>
                </div>
            </div>
        </div>
